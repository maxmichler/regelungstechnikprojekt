...
def poly_transition(tau, n=0, N=5):
    ...
    p = factorial(2*N+1)/factorial(N)**2 * \
        np.array([binom(N, k)*(-1)**k/(N+k+1) for k in range(N+1)])
...
def mimo_rnf(A, B, C, n):
    ...
    for ii in range(ninp):
        q = Sn_inv[nu[ii]-1, :]
        block = np.vstack([q@np.linalg.matrix_power(A, jj)
...
def mimo_acker(A, B, eigs):
    ...
    Arnf, Brnf, Crnf, M, Q, S = mimo_rnf(A, B, C, n)
